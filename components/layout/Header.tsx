          ...
          <div className="hidden md:flex items-center gap-3">
            {user ? (
              ...
              <>
                ...
                <Link href="/register">
                  <Button size="sm" className="bg-[#16a085] hover:bg-[#12876e] border-transparent">Register</Button>
                </Link>
              </>
            )}
          </div>
      ...
      {menuOpen && (
        <div className="md:hidden bg-[#0a2444] border-t border-white/10 px-4 pb-4">
          <nav className="flex flex-col gap-3 pt-3">
            {navLinks.map((l) => (
              <Link key={l.href} href={l.href} className="text-white/80 hover:text-white py-2 text-sm" onClick={() => setMenuOpen(false)}>
                {l.label}
              </Link>
            ))}
            <div className="flex flex-col gap-2 pt-2 border-t border-white/10">
              {user ? (
                <>
                  <p className="text-white/60 text-xs">{user.displayName || user.email}</p>
                ...
                <>
                  ...
                  <Link href="/register" onClick={() => setMenuOpen(false)}>
                    <Button size="sm" className="w-full bg-[#16a085] hover:bg-[#12876e] border-transparent">Register</Button>
                  </Link>
                </>
              )}
            </div>
